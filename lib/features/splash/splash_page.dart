...
class _SplashPageState extends ConsumerState<SplashPage>
  ...
  Future<void> _navigate() async {
    ...
    if (!mounted) return;
    switch ((auth as dynamic).runtimeType.toString()) {
      case 'AuthAuthenticated':
        final a = auth as AuthAuthenticated;
        if (a.user.role.name == 'admin') {
          context.go('/home/admin');
        } else if (a.user.isStaffOrAdmin) {
      ...
      body: RepaintBoundary(
        child: AnimatedBuilder(
          animation: Listenable.merge(
              [_spinCtrl, _pulseCtrl, _ecgCtrl, _appearCtrl, _glowCtrl]),
          builder: (context, _) {
            return Stack(
              children: [
...
class _SplashPainter extends CustomPainter {
  ...
  static const _particleSeeds = [
    (0.12, 0.22, 1.0), (0.88, 0.15, 0.8), (0.25, 0.65, 0.6),
    (0.75, 0.72, 0.9), (0.05, 0.50, 0.7), (0.92, 0.44, 0.5),
    (0.40, 0.10, 1.0), (0.60, 0.88, 0.6), (0.18, 0.85, 0.8),
    (0.82, 0.30, 0.7), (0.50, 0.05, 0.5), (0.35, 0.75, 0.9),
    (0.70, 0.50, 0.4), (0.15, 0.40, 0.6), (0.85, 0.78, 0.8),
  ];
  ...
  void _drawOrbitRings(Canvas canvas, Size size, Offset center) {
    for (int i = 1; i <= 5; i++) {
      ...
      final paint = Paint()
        ..color = AppColors.trustTeal
            .withAlpha(((opacity + opacity * 0.5 * glowIntensity) * 255).toInt().clamp(0, 255))
    ...
    final glowPaint = Paint()
      ..color = AppColors.nairaGreen.withAlpha((40 + 30 * glowIntensity).toInt())
      ..maskFilter = const MaskFilter.blur(BlurStyle.normal, 20);
    canvas.drawCircle(center, size.shortestSide * 0.22 * (1 + 0.03 * pulse),
        glowPaint);
  }
  ...
  void _drawParticles(Canvas canvas, Size size, Offset center) {
    for (final (normX, normY, speed) in _particleSeeds) {
      ...
      final y = ((normY - rotation / (2 * math.pi) * speed * 0.3) % 1.0) * size.height;
  ...
  void _drawNeedle(Canvas canvas, Size size, Offset center) {
    ...
    // ---- outer glow ----
    final glowPaint = Paint()
      ..color = AppColors.nairaGreen.withAlpha((55 + 40 * glowIntensity).toInt())
  ...
  Path _buildPlungerPath(double r) {
    ...
    path.addRRect(RRect.fromRectAndRadius(
        Rect.fromLTWH(-r * 0.15 - 30, -10, 8, 20), const Radius.circular(3)));
    return path;
  }
  ...
  void _drawEcg(Canvas canvas, Size size) {
    ...
    const pts = [
      (0.00, 0.00), (0.06, 0.00), (0.10, -0.08), (0.14, 0.00),
      (0.22, 0.00), (0.26, -0.12), (0.28, -0.85),
      (0.30, -1.00), (0.32, 0.40), (0.34, 0.00),
      (0.40, 0.00), (0.46, -0.18), (0.52, 0.00),
      (0.65, 0.00), (1.00, 0.00),
    ];
    ...
    canvas.drawRect(
        Rect.fromLTWH(0, baseY - amp * 1.5, w, amp * 3), fadePaint);
  }
